...
const IMAGE_VIEW_FIELDS = Object.values(ProductImageView).map((view) => ({ name: view, maxCount: 1 }));
...
export class ProductController {
	...
	@ApiBody({
		description: 'Standard OR NAFDAC payload depending on the category of `categoryId`.',
		schema: {
			oneOf: [{ $ref: getSchemaPath(CreateStandardProductDto) }, { $ref: getSchemaPath(CreateNafdacProductDto) }],
		},
	})
	@ApiResponse({ status: HttpStatus.CREATED, description: 'Product draft created.', type: Product })
	@ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Validation failed or NAFDAC lookup rejected the number.' })
	@ApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Missing or invalid token.' })
	@ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Category not found, or caller has no store profile yet.' })
	async create(@CurrentUser() user: { userId: string }, @Body() payload: unknown) {
	...
	@ApiResponse({ status: HttpStatus.OK, description: 'Paginated product cards.' })
	...
	@ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Malformed or unknown NAFDAC number.' })
	...
	@ApiBody({
		schema: {
			...
			properties: Object.fromEntries(
				IMAGE_VIEW_FIELDS.map(({ name }) => [name, { type: 'string', format: 'binary', description: `${name} view image` }]),
			),
			required: IMAGE_VIEW_FIELDS.map(({ name }) => name),
		},
	})
	@ApiResponse({ status: HttpStatus.ACCEPTED, description: 'Images accepted; 3D generation queued.', type: Product })
	@ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Missing/duplicate views, unsupported type or size > 5MB.' })
	@ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Product not found or not owned by you.' })
	async attachImages(
		@CurrentUser() user: { userId: string; roles: UserRole[] },
		@Param('id') productId: string,
		@UploadedFiles() files: Partial<Record<ProductImageView, Express.Multer.File[]>>,
	) {
		return this.productImageService.attachImages(user.userId, user.roles, productId, files ?? {});
	}
	...
	@ApiOperation({
		...
		description: 'Returns `model3dStatus` (PENDING → PROCESSING → COMPLETE | FAILED) and the model URL once COMPLETE.',
	})
	@ApiResponse({ status: HttpStatus.OK, description: 'Current 3D generation state.' })
	...
	@ApiResponse({ status: HttpStatus.OK, description: 'Updated product.', type: Product })
	@ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Product not found or not owned by you.' })
	async update(
		@CurrentUser() user: { userId: string; roles: UserRole[] },
		@Param('id') productId: string,
	...
	@ApiOperation({
		...
		description: 'Marks the product deleted — it disappears from listings while history is retained.',
	})
	@ApiResponse({ status: HttpStatus.OK, description: 'Product deleted.' })
	@ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Product not found or not owned by you.' })
	async remove(@CurrentUser() user: { userId: string; roles: UserRole[] }, @Param('id') productId: string) {
		await this.assertOwnership(user, productId);
		await this.productService.softDelete(productId);
		return { message: 'Product deleted.' };
	}

	private async assertOwnership(user: { userId: string; roles: UserRole[] }, productId: string): Promise<void> {
		const vendor = await this.vendorService.findByUserIdOrThrow(user.userId);
		const product = await this.productService.findByIdOrThrow(productId);
		this.productService.assertOwnership(product, vendor._id.toString(), user.roles.includes(UserRole.ADMIN));
	}
}
