...
export class NafdacProductCreationStrategy extends ProductCreationStrategy {
	...
	async create({ vendorId, category, payload }: ProductCreationInput) {
		...
		if (!result) {
			throw new BadRequestException(`NAFDAC number ${dto.nafdacNumber} was not found in the registry.`);
		}
		if (!result.isValid) {
			throw new BadRequestException(`NAFDAC number ${dto.nafdacNumber} is registered but not valid.`);
		}
