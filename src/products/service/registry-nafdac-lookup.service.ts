...
export class RegistryNafdacLookupService extends NafdacLookupService {
  ...
  constructor(
    ...
    super();
    this.baseUrl = (
      configService.get<string>('NAFDAC_REGISTRY_BASE_URL') ??
      RegistryNafdacLookupService.DEFAULT_BASE_URL
    ).replace(/\/+$/, '');
  }
  ...
  async lookup(nafdacNumber: string): Promise<NafdacLookupResult | null> {
    ...
    try {
      ...
      this.logger.error(
        `NAFDAC Greenbook lookup failed for ${query}`,
