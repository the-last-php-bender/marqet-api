...
/**
 * Deterministic mock for local dev and demos. A number is valid when it
 * matches the NAFDAC shape `<category code><form>-<4 to 6 digits>`
 * (e.g. A1-12345, B7-123456, C2-1234); expiry is synthesised 18 months out.
 */
...
export class MockNafdacLookupService extends NafdacLookupService {
  lookup(nafdacNumber: string): Promise<NafdacLookupResult | null> {
    const normalized = nafdacNumber.trim().toUpperCase();
    if (!VALID_NAFDAC_PATTERN.test(normalized)) return Promise.resolve(null);
