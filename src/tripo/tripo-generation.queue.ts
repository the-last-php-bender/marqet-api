...
export class TripoGenerationQueue implements ModelGenerationQueue {
	constructor(@InjectQueue(TRIPO_GENERATION_QUEUE) private readonly queue: Queue<TripoGenerationJobData>) {}

	async enqueueGeneration(productId: string, images: ImageRef[]): Promise<void> {
...
export function orderByView(images: ImageRef[]): string[] {
	const byView = new Map<ProductImageView, string>(images.map((image) => [image.view, image.url]));
	return MULTIVIEW_ORDER.map((view) => {
		const url = byView.get(view as ProductImageView);
		if (!url) throw new Error(`Cannot enqueue generation: missing "${view}" view image.`);
		return url;
	});
}
