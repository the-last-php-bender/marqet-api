...
import { Model3dStatus, NotificationEvent, ProductStatus } from '../../common/constants/enums';
...
export class TripoProcessor extends WorkerHost {
	...
	async process(job: Job<TripoGenerationJobData>): Promise<void> {
		...
		this.logger.log(`Processing 3D generation for product ${productId} (attempt ${job.attemptsMade + 1})`);

		const taskId = await this.tripoClient.createMultiviewTask(orderByView(images));
	...
	async onFailed(job: Job<TripoGenerationJobData>, error: Error): Promise<void> {
		const attemptsAllowed = job.opts.attempts ?? 1;
		if (job.attemptsMade < attemptsAllowed) return; // will retry — keep current state

		this.logger.error(`3D generation failed permanently for product ${job.data.productId}: ${error.message}`);
		try {
			await this.productsService.markGenerationFailed(job.data.productId);
			await this.notifySeller(job.data.productId);
		} catch (cleanupError) {
			this.logger.error(`Cleanup after failure also failed for ${job.data.productId}`, cleanupError as Error);
		}
	}
	...
	private async notifySeller(productId: string): Promise<void> {
		...
		try {
			...
			this.logger.error(`Notification failed for product ${productId}`, error as Error);
		}
	}
}
