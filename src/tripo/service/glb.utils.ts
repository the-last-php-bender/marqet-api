...
export interface GltfJson {
	...
	nodes?: Array<{ scale?: [number, number, number]; children?: number[]; mesh?: number; name?: string }>;
...
export function parseGlb(buffer: Buffer): { json: GltfJson; bin: Buffer | null } {
	...
	while (offset + CHUNK_HEADER_BYTES <= buffer.length) {
		...
		if (chunkType === CHUNK_TYPE_JSON) {
			json = JSON.parse(buffer.subarray(dataStart, dataStart + chunkLength).toString('utf8')) as GltfJson;
...
export function serializeGlb(json: GltfJson, bin: Buffer | null): Buffer {
	...
	if (jsonPadding > 0) jsonBuffer = Buffer.concat([jsonBuffer, Buffer.alloc(jsonPadding, 0x20)]);
