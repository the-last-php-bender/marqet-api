...
import { ApplyScaleInput, MeshBounds, MeshInspector, MeshTransformer } from '../interfaces/mesh.interface';
import { StorageProvider } from '../../storage/interfaces/storage-provider.interface';

const CONTENT_TYPE_GLB = 'model/gltf-binary';

async function fetchGlb(httpService: HttpService, modelUrl: string): Promise<Buffer> {
	const response = await firstValueFrom(
		httpService.get<ArrayBuffer>(modelUrl, { responseType: 'arraybuffer', timeout: 60_000 }),
	);
...
export class GlbMeshInspector extends MeshInspector {
	...
	async getBounds(modelUrl: string): Promise<MeshBounds> {
		...
		if (!Number.isFinite(minX)) throw new Error('GLB contains no usable POSITION accessors.');
...
export class GlbMeshTransformer extends MeshTransformer {
	...
	async applyScale({ modelUrl, scale, destinationKey }: ApplyScaleInput): Promise<string> {
		if (!Number.isFinite(scale) || scale <= 0) throw new Error(`Invalid mesh scale: ${scale}`);

		const buffer = await fetchGlb(this.httpService, modelUrl);
		const { json, bin } = parseGlb(buffer);
		if (!bin) throw new Error('GLB has no BIN chunk; cannot rescale vertex data.');
		...
		this.logger.log(`Scaled mesh uploaded to ${url} (scale=${scale.toFixed(4)})`);
		return url;
	}
	...
	private scaleAllPositions(json: GltfJson, bin: Buffer, scale: number): void {
		...
		for (const accessorIndex of this.positionAccessorIndexes(json)) {
			...
			if (accessor.componentType !== COMPONENT_TYPE_FLOAT || accessor.type !== TYPE_VEC3) {
				throw new Error(
			...
			if (!bufferView) throw new Error('POSITION accessor is missing its bufferView.');

			const stride = bufferView.byteStride && bufferView.byteStride > 0 ? bufferView.byteStride : 12;
