...
export class TripoClientService {
	...
	constructor(
		...
		this.baseUrl = configService.getOrThrow<string>('TRIPO_API_BASE_URL').replace(/\/+$/, '');
		this.apiKey = configService.getOrThrow<string>('TRIPO_API_KEY');
		this.pollIntervalMs = configService.get<number>('TRIPO_POLL_INTERVAL_MS') ?? 5000;
		this.pollTimeoutMs = configService.get<number>('TRIPO_POLL_TIMEOUT_MS') ?? 900_000;
	}
	...
	async waitForModel(taskId: string): Promise<string> {
		...
		while (Date.now() < deadline) {
			...
			if (status.status && TERMINAL_FAILURE_STATUSES.has(status.status)) {
				throw new Error(`Tripo task ${taskId} ended with status "${status.status}".`);
			}

			this.logger.debug(`Tripo task ${taskId}: status=${status.status} progress=${status.progress ?? 0}`);
	...
	private async fetchTask(taskId: string): Promise<TripoTaskResponse['data']> {
		try {
			const response = await firstValueFrom(
				this.httpService.get<TripoTaskResponse>(`${this.baseUrl}/task/${taskId}`, {
	...
	private assertTripoOk(body: TripoTaskResponse): void {
		if (body.code !== 0) throw new Error(body.message || `Tripo error code ${body.code}`);
	}

	private toUnprocessable(message: string, error: unknown): Error {
		if (error instanceof AxiosError && error.response) {
			return new Error(`${message} [${error.response.status}] ${JSON.stringify(error.response.data).slice(0, 500)}`);
		}
