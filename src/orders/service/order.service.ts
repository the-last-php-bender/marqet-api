...
export class OrderService {
  ...
  async create(buyerId: string, dto: CreateOrderDto): Promise<OrderDocument> {
    ...
    if (
      products.length !== new Set(dto.items.map((item) => item.productId)).size
    ) {
    ...
    if (unavailable) {
      throw new BadRequestException(
        `Product "${unavailable.productName}" is not available for purchase yet.`,
      );
    }

    const items: OrderItem[] = dto.items.map((item) => {
      const product = products.find(
        (candidate) => candidate._id.toString() === item.productId,
      )!;
      return {
        product: product._id,
  ...
  async findListForUser(
    ...
    if (!user.roles.includes(UserRole.ADMIN)) {
      if (query.view === 'sales') {
        const vendor = await this.vendorService.findByUserIdOrThrow(
          user.userId,
        );
    ...
    return paginate({
      ...
      populate: [
        ...
        {
          path: 'items.product',
          select: 'productName images model3dUrl model3dStatus status',
        },
      ],
    });
  }
  ...
  private async assertSellsOnOrder(
    order: OrderDocument,
    user: RequestingUser,
  ): Promise<void> {
    try {
