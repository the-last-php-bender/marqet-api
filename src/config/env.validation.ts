...
export class EnvironmentVariables {
	...
	@ValidateIf((o: EnvironmentVariables) => o.NAFDAC_PROVIDER === 'registry', {
		message: 'NAFDAC_REGISTRY_BASE_URL is required when NAFDAC_PROVIDER=registry',
	})
	@IsString()
	...
	@ValidateIf(
		(o: EnvironmentVariables) => !!o.ADMIN_SEED_EMAIL,
		{
...
function assertValidRs256Key(base64Key: string, name: string): void {
	...
	if (!pem.includes('-----BEGIN')) {
		throw new Error(`${name} does not decode to a PEM key. Base64-encode the full .pem file contents.`);
	}
}
