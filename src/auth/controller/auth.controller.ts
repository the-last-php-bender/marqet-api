...
export class AuthController {
	...
	@ApiResponse({ status: HttpStatus.TOO_MANY_REQUESTS, description: 'Rate limit exceeded (10/min per IP).' })
	...
	@Throttle({ default: { limit: 10, ttl: 60_000 } })
	@ApiOperation({ summary: 'Log in', description: 'Returns a fresh RS256 JWT for the account.' })
	@ApiResponse({ status: HttpStatus.OK, description: 'Authenticated.', type: AuthResponseDto })
	...
	@ApiOperation({ summary: 'Current authenticated user', description: 'Reads the live DB record, so role/profile changes are always current.' })
	@ApiResponse({ status: HttpStatus.OK, description: 'The authenticated user.' })
